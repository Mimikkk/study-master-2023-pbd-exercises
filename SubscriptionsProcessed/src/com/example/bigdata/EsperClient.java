...
public class EsperClient {
  public static void main(String[] args) throws InterruptedException {
    ...
    EPStatement meanwhile = runtime.getDeploymentService().getStatement(deployment.getDeploymentId(), "meanwhile");
    ...
    if (meanwhile != null) meanwhile.addListener((newData, oldData, stmt, runTime) -> {
      for (EventBean eventBean : newData) {
        System.out.printf("M: %s%n", eventBean.getUnderlying());
      }
    });
...
//        Random generator = new Random();
    ...
    while (System.currentTimeMillis() < startTime + (1000L * howLongInSec)) {
      ...
      for (int i = 0; i < noOfRecordsPerSec; i++) {
        ...
        var schema = Schema.of(
            ...
            Field.field("service", Service::random),
  ...
  public static String loadAnswer(int task) {
    try {
      return Files.readString(Paths.get(EsperClient.class.getClassLoader().getResource("answers/answer" + task + ".epl").toURI()));
